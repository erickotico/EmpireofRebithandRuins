...
function createMonsterModal(monster) {
    ...
    // extras dinâmicos
    const extraTabs = [];
    if (Array.isArray(monster.variantes) && monster.variantes.length) extraTabs.push({ key: 'variantes', label: 'Variantes' });
    if (monster.corrompido) extraTabs.push({ key: 'corrompido', label: 'Corrompido' });
    if (monster.zombie) extraTabs.push({ key: 'zumbi', label: 'Zumbi' });
    ...
    // resistencias, skills, rewards (uso do código já existente)
    ...
    const variantesHtml = (monster.variantes || []).map(v => `
    ...
    // Corrompido / Zumbi (se existir)
    const corrompidoHtml = monster.corrompido ? `
    ...
    const zumbiHtml = monster.zombie ? `
        <div class="variant-card single">
        ...
        <div class="monster-modal">
            <div class="modal-content" role="dialog" aria-modal="true" aria-label="${monster.name}">
                ...
                <div class="modal-body">
                    ...
                    ${ extraTabs.includes(item => item.key === 'variantes') ? '' : '' }
